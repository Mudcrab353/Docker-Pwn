...
echo -e "\033[92mPPPwn:\033[93m Python pppwn.py \033[0m" |  tee /dev/tty1
if [ $VMUSB = true ] ; then
...
done < <(timeout $TIMEOUT  python3 /boot/firmware/PPPwn/pppwn.py --interface=$INTERFACE --fw=${FIRMWAREVERSION//.} --ipv=$PYIP --gh=$PYGH)
if [[ $LEDACT == "status" ]] ;then
 	echo none |  tee $ALED >/dev/null
 	echo default-on |  tee $PLED >/dev/null
fi
